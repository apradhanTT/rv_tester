...
public:
  ...
  struct iss_select_s {
    ...
    void init(AddrType c, const std::string d, uint64_t v, uint64_t p) {
      type = c;
      disasm = d;
      virt_addr = v;
      phys_addr = p;
  ...
  bool whisperMcmDecode(int hart, uint64_t time, uint64_t tag, uint64_t addr, unsigned size, bool& valid);
...
public:
  ...
  CVM_MESSENGER_procedure_call(whisperMcmDecode, bool(int hart, uint64_t time, uint64_t tag, uint64_t addr, unsigned size, bool& valid));
