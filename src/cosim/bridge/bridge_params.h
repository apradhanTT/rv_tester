...
inline uint64_t mmr_lo_addr() {
  uint32_t cluster_id = 0;
  return generate_cr_device_addr(cluster_id);
}
inline uint64_t mmr_hi_addr() {
  uint32_t cluster_id = 0;
  return generate_axisw_device_addr(cluster_id) + 0xFFFF;
}
...
constexpr uint64_t debug_mem_lo = 0x42190000;
constexpr uint64_t debug_mem_hi = 0x42191000;
...
inline uint64_t mtime_mmr() {
  uint32_t cluster_id = 0;
  return generate_acl_device_addr(cluster_id) + 0;
}
inline uint64_t mtimecmp_mmr() {
  uint32_t cluster_id = 0;
  return generate_acl_device_addr(cluster_id) + 0x8000;
}
