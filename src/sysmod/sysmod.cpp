...
void sysmod::timer_interrupt(clint::timer_t t) {
...
void sysmod::sw_interrupt(clint::sw_t s) {
  cvm::registry::callbacks.push(
      loc_,
      [s]() {
...
void sysmod::uc_helper_backdoor_write(uc_helper::uc_helper_write_t w) {
...
void sysmod::uc_helper_backdoor_read(uc_helper::uc_helper_read_req_t r) {
...
void sysmod::dmi_write(debugger::dmi_data_t i) {
  cvm::registry::callbacks.push(
      loc_,
      [i]() {
...
void sysmod::set_secure_region(std::string region) {
  if (region == "")
    return;
  stee = true;
...
sysmod::create_aplic() const {
  ...
  for (const auto& d : memmap_) {
    ...
    const std::optional<std::string> parent = parent_json.is_null() ? static_cast<std::optional<std::string>>(std::nullopt) : parent_json.template get<std::string>();
    const size_t child_index = attributes.at("child_index");
    const auto privilege = attributes.at("is_machine") ? TT_APLIC::Privilege::Machine : TT_APLIC::Privilege::Supervisor;
...
bool sysmod::bin_load(const std::string load, bool lz4_compressed) {
  uint64_t offset = 0;
  ...
  if (not dev("memory") or not(lz4_compressed ? dynamic_cast<sysmod_mem&>(*dev("memory")).init_lz4(file, offset) : dynamic_cast<sysmod_mem&>(*dev("memory")).init_bin(file, offset))) {
...
void sysmod::load_prog(const std::string& hex, const std::string& load, const std::string& lz4, const std::string& bin) {
...
void sysmod::load_boot(const std::string& boot) {
...
void sysmod::load_cplfw(const std::string& cplfw) {
...
void sysmod::tick(uint64_t advance) {
...
void sysmod::configure_uninit_read_callbacks() {
  ...
  if (FLAGS_sysmod_mem_random) {
    callback = [this](std::uint64_t addr, std::uint64_t size) -> std::vector<std::uint8_t> {
      ...
      std::generate(data.begin(), data.end(), [&random_byte_dist]() { return random_byte_dist(); });
